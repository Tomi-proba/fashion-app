...
export default function Generator({
  ...
  function selectLook(tag: StyleTag) {
    const next = look === tag ? null : tag
    setLook(next)
    if (next === null) setShowSuggestion(false)
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      ...
      <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
        Pick a look for an instant suggestion, or combine it with an anchor piece and what to include/avoid to
        generate from your own wardrobe.
      </p>

      <section className="mt-6">
        <h2 className="text-sm font-semibold text-neutral-800 dark:text-neutral-200">1. Pick a look</h2>
        <div className="mt-2 flex flex-wrap gap-2">
          ...
          />
          {STYLE_TAGS.map((tag) => (
            <Chip key={tag} label={STYLE_DEFS[tag].label} selected={look === tag} onClick={() => selectLook(tag)} />
          ))}
        </div>
        ...
        <button
          ...
          disabled={!look}
          className="mt-4 rounded-full border border-neutral-300 px-4 py-2 text-sm font-semibold text-neutral-800 hover:bg-neutral-100 disabled:cursor-not-allowed disabled:opacity-40 dark:border-neutral-700 dark:text-neutral-200 dark:hover:bg-neutral-800"
        >
          ✨ Suggested outfit & colors
        </button>
        {!look && <p className="mt-2 text-xs text-neutral-400">Pick a look above to get an instant suggestion — no wardrobe needed.</p>}

        {look && showSuggestion && (
          <div className="mt-4">
            <StyleSuggestionCard look={look} />
          </div>
        )}
      </section>
      ...
      <section className="mt-8 border-t border-neutral-100 pt-6 dark:border-neutral-800">
        ...
        <section className="mt-6">
          ...
          <div className="mt-2 flex max-h-32 flex-wrap gap-2 overflow-y-auto rounded-lg border border-neutral-100 p-2 dark:border-neutral-800">
            {owned.length === 0 && <span className="text-xs text-neutral-400">No wardrobe items yet.</span>}
            {owned.map((item) => (
              <Chip
                key={item.id}
                label={item.name}
        ...
        <button
          ...
          disabled={owned.length === 0}
          className="mt-8 w-full rounded-full bg-neutral-900 px-4 py-3 text-sm font-semibold text-neutral-50 hover:bg-neutral-800 disabled:cursor-not-allowed disabled:opacity-40 dark:bg-neutral-100 dark:text-neutral-900 dark:hover:bg-neutral-300"
        >
